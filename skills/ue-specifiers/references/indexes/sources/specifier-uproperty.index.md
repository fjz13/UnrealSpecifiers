...
|---|---|---|---|
| `AdvancedDisplay` | 被折叠到高级栏下，要手动打开。一般用在不太常用的属性上面 | UPROPERTY / DetailsPanel | `DetailsPanel/AdvancedDisplay.md` |
...
| `Category` | 指定属性的类别，使用 &#124; 运算符定义嵌套类目 | UPROPERTY / DetailsPanel | `DetailsPanel/Category.md` |
...
| `EditAnywhere` | 在默认值和实例的细节面板上均可编辑 | UPROPERTY / DetailsPanel | `DetailsPanel/EditAnywhere.md` |
| `EditDefaultsOnly` | 只能在默认值面板里编辑 | UPROPERTY / DetailsPanel | `DetailsPanel/EditDefaultsOnly.md` |
| `EditFixedSize` | 在细节面板上不允许改变该容器的元素个数 | UPROPERTY / DetailsPanel | `DetailsPanel/EditFixedSize.md` |
| `EditInstanceOnly` | 只能在实例的细节面板上编辑该属性 | UPROPERTY / DetailsPanel | `DetailsPanel/EditInstanceOnly.md` |
...
| `Interp` | 指定该属性值可暴露到时间轴里编辑，在平常的Timeline或UMG的动画里使用 | UPROPERTY / DetailsPanel | `DetailsPanel/Interp.md` |
...
| `NoClear` | 指定该属性的编辑选项中不出现Clear按钮，不允许置null | UPROPERTY / DetailsPanel | `DetailsPanel/NoClear.md` |
| `NonPIEDuplicateTransient` | 在对象复制的时候，且在不是PIE的场合，忽略该属性 | UPROPERTY / Serialization | `Serialization/NonPIEDuplicateTransient.md` |
| `NonTransactional` | 对该属性的改变操作，不会被包含进编辑器的Undo/Redo命令中 | UPROPERTY / DetailsPanel | `DetailsPanel/NonTransactional.md` |
...
| `SimpleDisplay` | 在细节面板中直接可见，不折叠到高级中 | UPROPERTY / DetailsPanel | `DetailsPanel/SimpleDisplay.md` |
...
| `VisibleAnywhere` | 在默认值和实例细节面板均可见，但不可编辑 | UPROPERTY / DetailsPanel | `DetailsPanel/VisibleAnywhere.md` |
| `VisibleDefaultsOnly` | 在默认值细节面板可见，但不可编辑 | UPROPERTY / DetailsPanel | `DetailsPanel/VisibleDefaultsOnly.md` |
| `VisibleInstanceOnly` | 在实例细节面板可见，但不可编辑 | UPROPERTY / DetailsPanel | `DetailsPanel/VisibleInstanceOnly.md` |
