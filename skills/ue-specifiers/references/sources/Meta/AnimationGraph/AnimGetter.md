...
summary: "指定UAnimInstance及子类的该函数成为一个AnimGetter函数"
usage: "UFUNCTION"
---
...
- **Use when:** `AnimGetter` metadata 的 `UFUNCTION` 场景需要：指定UAnimInstance及子类的该函数成为一个AnimGetter函数。
- **Do not use when:** 目标声明不属于 `UFUNCTION`，或该展示/编辑/节点行为不是代码契约的一部分。
- **Requires:** 函数通常在 `UAnimInstance` 子类中，配合 `BlueprintPure`、`BlueprintInternalUseOnly=true`、`BlueprintThreadSafe` 和可自动填充的索引参数。
...
- **关联项：** [GetterContext](GetterContext.md)
- **常用程度：** ★★★
