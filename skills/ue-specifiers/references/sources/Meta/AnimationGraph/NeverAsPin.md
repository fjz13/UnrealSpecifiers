...
summary: "在动画蓝图中使得动画节点的某个属性总是不暴露出来成为引脚"
usage: "UPROPERTY"
---
...
- **Use when:** `NeverAsPin` metadata 的 `UPROPERTY` 场景需要：在动画蓝图中使得动画节点的某个属性总是不暴露出来成为引脚。
...
- **元数据类型：** bool
- **限制类型：** FAnimNode_Base
- **关联项：** [PinShownByDefault](PinShownByDefault.md)
- **常用程度：** ★★★
