...
- **Use when:** `BlueprintInternalUseOnly` metadata 的 `UFUNCTION, USTRUCT` 场景需要：标明该元素是作为蓝图系统的内部调用或使用，不暴露出来在用户层面直接定义或使用。
- **Do not use when:** 目标声明不属于 `UFUNCTION, USTRUCT`，或该展示/编辑/节点行为不是代码契约的一部分。
