...
id: "meta.BlueprintThreadSafe"
kind: "meta"
symbol: "BlueprintThreadSafe"
category: "Blueprint"
source_status: "verified_UE5.8"
target_ue_version: "UE5.8"
normalization_status: "normalized"
normalized_at: "2026-06-04"
summary: "用在类上或函数上，标记类里的函数都是线程安全的"
...
- **Use when:** `BlueprintThreadSafe` metadata 的 `UCLASS, UFUNCTION` 场景需要：用在类上或函数上，标记类里的函数都是线程安全的。
- **Do not use when:** 目标声明不属于 `UCLASS, UFUNCTION`，或该展示/编辑/节点行为不是代码契约的一部分。
- **Requires:** 被标记的函数必须真实线程安全，避免访问 mutable UObject 状态、World、latent/game-thread-only API 或其他非线程安全资源。
...
UE5.8 class/function metadata；BlueprintGraph `BlueprintThreadSafe`，启用 thread-safety checks。
