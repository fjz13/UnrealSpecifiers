---
title: "IgnoreTypePromotion"
id: "meta.IgnoreTypePromotion"
kind: "meta"
...
- **Use when:** `IgnoreTypePromotion` metadata 的 `UFUNCTION` 场景需要：标记该函数不收录进类型提升函数库。
- **Do not use when:** 目标声明不属于 `UFUNCTION`，或该展示/编辑/节点行为不是代码契约的一部分。
- **Requires:** 仅用于需要从 Blueprint type promotion discovery 排除的函数；常见目标是函数库中的纯运算/Make/Break 风格函数。
