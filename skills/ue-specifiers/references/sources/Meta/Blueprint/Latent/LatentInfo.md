...
summary: "和Latent配合，指明哪个函数参数是LatentInfo参数"
usage: "UFUNCTION"
---
...
- **Use when:** `LatentInfo` metadata 的 `UFUNCTION` 场景需要：和Latent配合，指明哪个函数参数是LatentInfo参数。
- **Do not use when:** 目标声明不属于 `UFUNCTION`，或该展示/编辑/节点行为不是代码契约的一部分。
- **Requires:** 函数同时使用 `Latent`，签名包含 `FLatentActionInfo` 参数，且 metadata 值命中该参数名。
...
UE5.8 function metadata；指定 latent info 参数。
