...
summary: "指定一个函数为使用Array<*>的函数，数组元素类型为通配符的泛型"
usage: "UFUNCTION"
---
...
- **Use when:** `ArrayParm` metadata 的 `UFUNCTION` 场景需要：指定一个函数为使用Array<*>的函数，数组元素类型为通配符的泛型。
...
- **引擎模块：** Blueprint
- **元数据类型：** strings="a，b，c"
- **关联项：** [ArrayTypeDependentParams](ArrayTypeDependentParams.md)
- **常用程度：** ★★★
