...
- **Use when:** `EditInline` metadata 的 `UPROPERTY` 场景需要：为对象属性创建一个实例，并作为子对象。
- **Do not use when:** 目标声明不属于 `UPROPERTY`，或该展示/编辑/节点行为不是代码契约的一部分。
- **Requires:** 只控制 Details Panel inline 展示；owned inline UObject 通常还需要 `Instanced` 和类侧 `EditInlineNew`。
...
- **关联项：** [NoEditInline](NoEditInline.md), [AllowEditInlineCustomization](AllowEditInlineCustomization.md), [ForceInlineRow](ForceInlineRow.md)
...
UE5.8 property metadata；UHT `Instanced` 会写入 `EditInline=true`，PropertyEditor inline object 路径读取。
