...
- **Use when:** `DeprecationMessage` metadata 的 `UCLASS, UFUNCTION, UPROPERTY` 场景需要：定义弃用的消息。
- **Do not use when:** 目标声明不属于 `UCLASS, UFUNCTION, UPROPERTY`，或该展示/编辑/节点行为不是代码契约的一部分。
