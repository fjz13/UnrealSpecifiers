...
summary: "设定数字输入框值改变的幅度为Delta的倍数"
usage: "UPROPERTY"
---
...
- **Use when:** `Delta` metadata 的 `UPROPERTY` 场景需要：设定数字输入框值改变的幅度为Delta的倍数。
...
UE5.8 numeric metadata；ObjectMacros 标注为 slider/spinbox step size。
