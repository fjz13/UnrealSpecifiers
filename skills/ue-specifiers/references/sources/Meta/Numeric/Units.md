...
summary: "设定属性值的单位，支持实时根据数值不同动态改变显示的单位"
usage: "UPROPERTY"
---
...
- **Use when:** `Units` metadata 的 `UPROPERTY` 场景需要：设定属性值的单位，支持实时根据数值不同动态改变显示的单位。
...
UE5.8 numeric metadata；ObjectMacros 标注为内部单位，Details 可按用户偏好/量级显示。
