...
summary: "使用UE的风格来选择Content以及子目录"
usage: "UPROPERTY"
---
...
- **Use when:** `ContentDir` metadata 的 `UPROPERTY` 场景需要：使用UE的风格来选择Content以及子目录。
- **Do not use when:** 目标声明不属于 `UPROPERTY`，或该展示/编辑/节点行为不是代码契约的一部分。
- **Requires:** 目标属性应是 `FDirectoryPath`；该 metadata 只影响编辑器路径选择器和路径格式。
