...
summary: "指定一个函数参数为DataTable或CurveTable类型，以便为FName的其他参数提供RowNameList的选择"
usage: "UFUNCTION"
---
...
- **Use when:** `DataTablePin` metadata 的 `UFUNCTION` 场景需要：指定一个函数参数为DataTable或CurveTable类型，以便为FName的其他参数提供RowNameList的选择。
...
UE5.8 function metadata；BlueprintGraph 使用 DataTable pin 决定 RowName 选项。
