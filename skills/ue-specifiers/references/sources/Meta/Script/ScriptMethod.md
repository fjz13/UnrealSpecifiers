...
- **Use when:** `ScriptMethod` metadata 的 `UFUNCTION` 场景需要：把静态函数导出变成第一个参数的成员函数。
- **Do not use when:** 目标声明不属于 `UFUNCTION`，或该展示/编辑/节点行为不是代码契约的一部分。
- **Requires:** 静态函数需要符合脚本/Python export 的 receiver 形态；第一个参数会被当作脚本方法宿主。
