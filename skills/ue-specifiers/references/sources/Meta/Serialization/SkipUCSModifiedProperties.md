---
title: "SkipUCSModifiedProperties"
id: "meta.SkipUCSModifiedProperties"
kind: "meta"
...
summary: "允许ActorComponent里的属性在Actor构造函数里被修改后依然保存下来"
usage: "UPROPERTY"
---
...
- **Use when:** `SkipUCSModifiedProperties` metadata 的 `UPROPERTY` 场景需要：允许ActorComponent里的属性在Actor构造函数里被修改后依然保存下来。
- **Do not use when:** 目标声明不属于 `UPROPERTY`，或该展示/编辑/节点行为不是代码契约的一部分。
- **Requires:** 用于组件/Actor construction script 修改属性的序列化处理路径；不要当成普通保存或复制规则。
