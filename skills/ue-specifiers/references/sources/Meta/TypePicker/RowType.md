...
summary: "指定FDataTableRowHandle 属性的可选行类型的基类"
usage: "UPROPERTY"
---
...
- **Use when:** `RowType` metadata 的 `UPROPERTY` 场景需要：指定FDataTableRowHandle 属性的可选行类型的基类。
- **Do not use when:** 目标声明不属于 `UPROPERTY`，或该展示/编辑/节点行为不是代码契约的一部分。
- **Requires:** 目标属性应是 `FDataTableRowHandle`，metadata 值应指向可解析的 row `UScriptStruct`。
