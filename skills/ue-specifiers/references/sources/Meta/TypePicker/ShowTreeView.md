...
summary: "用于选择Class或Struct的属性上，使得在类选取器中显示为树形而不是列表"
usage: "UPROPERTY"
---
...
- **Use when:** `ShowTreeView` metadata 的 `UPROPERTY` 场景需要：用于选择Class或Struct的属性上，使得在类选取器中显示为树形而不是列表。
