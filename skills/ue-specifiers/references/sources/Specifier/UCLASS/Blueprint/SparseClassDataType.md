...
summary: "让Actor的一些重复不变的数据存放在一个共同的结构里，以达到减少内容使用量的目的"
...
- **Use when:** `SparseClassDataType` specifier 的 `UCLASS / Blueprint` 场景需要：让Actor的一些重复不变的数据存放在一个共同的结构里，以达到减少内容使用量的目的。
