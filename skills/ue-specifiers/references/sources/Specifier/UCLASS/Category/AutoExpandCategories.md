...
id: "UCLASS.AutoExpandCategories"
kind: "specifier"
...
summary: "指定此类的对象在细节面板中应该自动展开的Category"
...
- **Use when:** `AutoExpandCategories` specifier 的 `UCLASS / Category` 场景需要：指定此类的对象在细节面板中应该自动展开的Category。
...
UE5.8 UHT 把指定分类加入 `AutoExpandCategories`，并从 `AutoCollapseCategories` 移除同名分类。
