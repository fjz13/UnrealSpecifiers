...
id: "UCLASS.ComponentWrapperClass"
kind: "specifier"
symbol: "ComponentWrapperClass"
scope: "UCLASS"
category: "Category"
...
summary: "指定该类为一个简单的封装类，忽略掉子类的Category相关设置"
...
- **Use when:** `ComponentWrapperClass` specifier 的 `UCLASS / Category` 场景需要：指定该类为一个简单的封装类，忽略掉子类的Category相关设置。
...
UE5.8 UHT 写入 `IgnoreCategoryKeywordsInSubclasses=true` metadata，用于组件 wrapper 类的分类继承处理。
