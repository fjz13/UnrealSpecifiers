...
id: "UFUNCTION.ServiceRequest"
kind: "specifier"
symbol: "ServiceRequest"
scope: "UFUNCTION"
category: "Network"
...
summary: "此函数为RPC（远程过程调用）服务请求。rpc服务请求"
...
- **Use when:** `ServiceRequest` specifier 的 `UFUNCTION / Network` 场景需要：此函数为RPC（远程过程调用）服务请求。rpc服务请求。
...
- **元数据类型：** bool
- **引擎模块：** Network
- **作用机制：** 在Meta中加入[CustomThunk](../../../Meta/UHT/CustomThunk.md)，在FunctionFlags加入FUNC_Net、FUNC_Event、FUNC_NetReliable、FUNC_NetRequest
...
在 UE5.8 UHT 中写入 `FUNC_Net | FUNC_NetReliable | FUNC_NetRequest` 和 custom thunk export flag，并要求 endpoint name。它是专门的 service RPC 语义，不是常规 Gameplay RPC 首选。
...
不写 endpoint；或把它当成普通 `Server` RPC 的替代品。
