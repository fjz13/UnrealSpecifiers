...
id: "UFUNCTION.BlueprintInternalUseOnly"
kind: "specifier"
...
summary: "指示不应向最终用户公开此函数。蓝图内部调用，不暴露给用户"
usage: "UFUNCTION / UHT"
---
...
- **Use when:** `BlueprintInternalUseOnly` specifier 的 `UFUNCTION / UHT` 场景需要：指示不应向最终用户公开此函数。蓝图内部调用，不暴露给用户。
