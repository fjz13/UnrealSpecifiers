...
summary: "为属性增加一个C++的Set函数，只在C++层面应用"
...
- **Use when:** `Setter` specifier 的 `UPROPERTY / Blueprint` 场景需要：为属性增加一个C++的Set函数，只在C++层面应用。
...
在 UE5.8 UHT 中写入 setter export flag，并支持 `None`、`Auto` 或自定义函数名。UHT 要求用于 class member。
...
和 `BlueprintSetter` 混淆；或在 struct member 上使用。
