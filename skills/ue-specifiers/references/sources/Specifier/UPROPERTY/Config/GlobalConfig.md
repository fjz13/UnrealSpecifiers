...
- **Use when:** `GlobalConfig` specifier 的 `UPROPERTY / Config` 场景需要：和Config一样指定该属性可作为配置读取和写入ini中，但只会读取写入到配置文件里基类的值，而不会使用配置文件里子类里的值。
- **Do not use when:** 声明宏、目标类型或代码契约不属于 `UPROPERTY / Config`。
