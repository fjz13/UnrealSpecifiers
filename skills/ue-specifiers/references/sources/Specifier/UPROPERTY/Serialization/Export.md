...
summary: "在对Asset导出的时候，决定该类的对象应该导出内部的属性值，而是对象的路径"
...
- **Use when:** `Export` specifier 的 `UPROPERTY / Serialization` 场景需要：在对Asset导出的时候，决定该类的对象应该导出内部的属性值，而是对象的路径。
...
在 UE5.8 UHT 中写入 `CPF_ExportObject`。它影响对象引用导出/文本导出相关路径，常和 instanced subobject 语义相邻，但不等同于 `Instanced`。
...
只写 `Export` 就期待内联创建子对象；通常应检查是否需要 `Instanced`/class `EditInlineNew`。
